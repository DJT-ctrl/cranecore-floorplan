...
export function normalizeAnalysis(
  ...
  // Trusted total area: prefer the model's reported totalAreaM2 (it is the
  // headline number in the UI). Fall back to the outline-derived footprint or
  // the summed room area when the model omits it.
  ...
  // Calibrate pxPerM against the trusted total so wall/glass area does not
  // collapse to zero when Gemini's inferred scale disagrees with its own
  // totalAreaM2. We use the largest reliable pixel reference available
  // (outline first, otherwise summed room polygons).
  ...
  // The "wall area = footprint − rooms" fallback is only safe when the
  // per-room areas are dense enough to actually represent the interior
  // (>= 60% of the footprint). Otherwise the model probably skipped per-room
  // areaM2, and subtracting tiny room totals would falsely claim the entire
  // plan is wall mass — which then collapses floor area to zero.
  ...
  // Floor area = footprint minus walls. When walls are unknown we surface the
  // full footprint instead of zero so the UI still reports usable floor area.
  ...
  // Persist the scale actually used for measurements so the response and
  // overlay stay consistent with the reported areas.
